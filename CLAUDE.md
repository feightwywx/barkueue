...
**`src/barkueue/__init__.py`** 是公开 API 入口。外部代码通过 `import barkueue as bark` 导入，调用 `bark.app()` 获取单例 `Application`。`app()` 支持参数：`sources`、`max_workers`、`queue_timeout`、`fetch_interval`。同时导出 `Event` 和六个事件名常量。
